...
export async function createAdminOrder(input: Parameters<typeof local.createAdminOrder>[0]) {
  const order = isRemoteBackend() ? await remote.createAdminOrder(input) : local.createAdminOrder(input);
  notifyStoreUpdate();
  return order;
}
